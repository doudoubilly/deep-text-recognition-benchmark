...
from dataset import hierarchical_dataset, AlignCollate, Batch_Balanced_Dataset, TxtDataset
from model import Model
from test import validation
os.environ['CUDA_VISIBLE_DEVICES'] = '0'


def random_rotate180(img):
...
class dataloader_iter(object):
    ...
    def get_batch(self):
        try:
            image, text = self.data_iter.next()
...
def train(opt):
    ...
    AlignCollate_0 = AlignCollate(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD)

    train_dataset = TxtDataset(opt.train_data, opt.train_txt, opt, random_rotate180)
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=opt.batch_size,
        shuffle=True, num_workers=int(opt.workers),
        collate_fn=AlignCollate_0, pin_memory=True
    )
    train_loader_iter = dataloader_iter(train_loader)

    # valid_dataset = hierarchical_dataset(root=opt.valid_data, opt=opt)
    valid_dataset = TxtDataset(opt.valid_data, opt.val_txt, opt)
    ...
    while(True):
        ...
        image_tensors, labels = train_loader_iter.get_batch()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_txt', required=True, help='path to training dataset')
    parser.add_argument('--val_txt', required=True, help='path to training dataset')
